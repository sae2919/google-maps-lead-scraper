{{-- ═══════════════════════════════════════════════════ --}}
{{-- GLOBAL SCRAPING STATUS BAR — include in every page --}}
{{-- ═══════════════════════════════════════════════════ --}}
<style>
...
<div id="globalScrapingBar"
    ...
    <div class="px-6 py-3 flex items-center justify-between gap-4 flex-wrap">
        ...
        <div class="flex items-center gap-3">
            ...
            "></span>
            <span class="text-sm font-black text-slate-700 uppercase tracking-wide">Scraping Running</span>
            <span id="globalQuery" class="text-xs text-slate-400 font-medium italic max-w-xs truncate"></span>
        </div>
        ...
        <div class="flex items-center gap-6 text-sm">
            ...
            <div class="flex flex-col items-center">
                ...
                <span class="font-black text-xs leading-tight" id="globalStatus" style="color:#16a34a;">RUNNING</span>
            </div>
        </div>
        ...
        <div class="flex items-center gap-2">
            ...
            <button onclick="document.getElementById('globalScrapingBar').classList.add('hidden')"
        ...
        fetch('/api/progress/' + activeSearchId)
            ...
            .then(function(data) {
                ...
                if (status === 'RUNNING') {
                    ...
                    statusEl.innerText     = status === 'COMPLETED' ? '✅ COMPLETED' : 'STOPPED';
                    statusEl.style.color   = status === 'COMPLETED' ? '#6366f1' : '#dc2626';
                    dotEl.style.background = status === 'COMPLETED' ? '#6366f1' : '#ef4444';
                    dotEl.style.animation  = 'none';
                    if (status === 'COMPLETED') {
